...
import { NameMovement, Bucket } from './store/names/types'
  ...
  if (MOCK_API) {
    ...
    return new Promise((resolve) => {
      const fakeData = {
        ...
        Counterpart: 'e3c38f5e-794b-4b33-a3f9-3de70d489335',
        StateId: '4c501f5a-efb2-4e48-ad17-31dbdea45bde',
        Email: 'sindrigudmundsson@gmail.com',
      }
      shuffle(fakeData.Remaining)
      resolve(fakeData)
    })
  }
  try {
  ...
  if (MOCK_API) {
    return new Promise((resolve) => {
      resolve({
        ...
        progress: {
          self: 0.3376818866031109,
          counterpart: 0.45760160561966884,
        },
      })
    })
  }
...
export const saveMovements = async (
  ...
  if (MOCK_API) {
    ...
    const select = movements
      .filter(({ to }) => to === Bucket.Selected)
      .map(({ name }) => name)
    const reject = movements
      .filter(({ to }) => to === Bucket.Rejected)
      .map(({ name }) => name)
    return new Promise((resolve) => {
      const fakeData = {
        Remaining: state.remaining
          .filter(
            (name) => select.indexOf(name) < 0 && reject.indexOf(name) < 0,
          )
          .toJS(),
...
export const createState = async (
  ...
  if (MOCK_API) {
    return new Promise((resolve) =>
      resolve({ stateId: 'd0391b6c-a5df-4597-993c-1dac2dca8bc4' }),
    )
  }
  try {
